...
function openFont($font){
  ...
  if (file_exists($dir.'php_'.$name.'.afm')){
    ...
    $this->fonts[$font]=unserialize(str_replace('\\','',$tmp[0]));  /*Cambio hecho por Luis Aylas (le puse el str_replace)*/
...
function setCurrentFont(){
  if (strlen($this->currentBaseFont)==0){
    ...
    $this->selectFont('system/application/libraries/fonts/Helvetica.afm');
  }
...
function getFontHeight($size){
  if (!$this->numFonts){
    $this->selectFont('system/application/libraries/fonts/Helvetica');
  }
...
function addTextWrap($x,$y,$width,$size,$text,$justification='left',$angle=0,$test=0){
  ...
  if (!$this->numFonts){$this->selectFont('system/application/libraries/fonts/Helvetica');}
  if ($width<=0){
